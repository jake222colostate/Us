...
const UserProfile = () => {
  ...
  const fallbackName = React.useMemo(() => profile?.user.name ?? "User", [profile?.user.name]);
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="relative">
        {heroImage ? (
          <img src={heroImage} alt={profile.user.name} className="h-[60vh] w-full object-cover" loading="React.lazy" />
        ) : (
          <Skeleton className="h-[60vh] w-full" />
        )}
